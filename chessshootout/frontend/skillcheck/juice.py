...
class Trauma:
    ...
    def __init__(self, decay_per_s=1.5):
...
class Hitstop:
    ...
    def __init__(self, cap_ms=250.0):
        self._cap_ms = cap_ms
        self._until = None
...
def _torn_surface(base, key, tier):
    ...
    for _ in range(_TORN_NOTCHES[tier]):
        ...
        reach = 0.85 + 0.25 * floats[idx + 2]
        _punch(mask, cx + math.cos(ang) * hx * reach, cy + math.sin(ang) * hy * reach,
               radius_base * (0.7 + 0.6 * floats[idx + 1]))
        idx += 3
    if tier >= 3:
        ang = floats[idx] * 2.0 * math.pi
        _punch(mask, cx + math.cos(ang) * hx * 0.6, cy + math.sin(ang) * hy * 0.6,
               span * _TORN_CHUNK_FRAC)
        idx += 1
        for _ in range(_TORN_RAGGED):
            _punch(mask, bbox.left + bbox.width * floats[idx],
                   bbox.top + span * 0.08 * floats[idx + 1], span * 0.09)
            idx += 2
    surf.blit(mask, (0, 0), special_flags=pg.BLEND_RGBA_MULT)
    _draw_cracks(surf, bbox, floats, idx, _TORN_CRACKS[tier], max(int(span * 0.03), 1))
    return surf
