...
def _e4(app):
    app.match.try_move(sq(6, 4), sq(4, 4))


def _e5(app):
    app.match.try_move(sq(1, 4), sq(3, 4))
...
# ---- OSError -> toast once + fallback dir + latch --------------------------
...
# ---- PermissionError on os.replace: transient, no latch --------------------
...
# ---- back-to-menu / teardown backstops --------------------------------------
...
def test_back_to_menu_during_result_confirm_window_force_saves_real_result(
    ...
    assert app.manual_result is None, "still waiting on the server confirm"
...
# ---- drain reorder: result / overlay / banner land on the SAME frame -------
...
# ---- filename collision within the same second -----------------------------
...
# ---- local resign/draw-agreement save at adoption, not the next frame ------

def test_local_resign_saves_immediately_without_a_follow_up_frame(tmp_path, monkeypatch):
    app = _local_app(tmp_path, monkeypatch)
    _e4(app)
    app._perform_resign()
    assert app.manual_result == "white_wins_by_resignation", "black (to move) resigned"
    files = _pgn_files(tmp_path)
...
# ---- online draw reasons keep their own subtitle ----------------------------
...
# ---- incremental autosave: throttle + "*" tag + finalize rewrite -----------
