...
def test_transposition_yields_same_key():
    # Two move orders that arrive at the same position must produce identical keys.
    a = Backend()
    a.new_game()
    a.try_move(sq(6, 4), sq(4, 4))  # e4
    a.try_move(sq(1, 4), sq(3, 4))  # e5
    a.try_move(sq(7, 6), sq(5, 5))  # Nf3
    a.try_move(sq(0, 1), sq(2, 2))  # Nc6

    b = Backend()
    b.new_game()
    b.try_move(sq(7, 6), sq(5, 5))  # Nf3
    b.try_move(sq(1, 4), sq(3, 4))  # e5
    b.try_move(sq(6, 4), sq(4, 4))  # e4
    b.try_move(sq(0, 1), sq(2, 2))  # Nc6

    assert a._position_key() == b._position_key()


def test_piece_instances_not_in_key():
    # The hash must not depend on Piece object identity. Different Piece instances
    # with the same (type, color) should produce identical keys.
