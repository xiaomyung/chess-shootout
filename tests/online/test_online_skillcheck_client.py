...
def test_online_gate_holds_a_capture_without_applying_locally():
    ...
    held = app.game.skillcheck_session._skillcheck_gate(frm, to)
    ...
    assert app.game.skillcheck_session._pending_online_move == (frm, to, None)
...
def test_online_gate_holds_a_promotion_with_the_chosen_letter():
    ...
    held = app.game.skillcheck_session._skillcheck_gate(frm, to, Q)
    assert held is True
    assert app.coordinator.client.sent_moves == [("a7", "a8", "q")], "promotion sent as a letter"
    assert app.game.skillcheck_session._pending_online_move[:2] == (frm, to)
...
def test_online_gate_ignores_an_illegal_destination():
    ...
    held = app.game.skillcheck_session._skillcheck_gate(frm, sq(0, 0))
...
def test_required_opens_a_neutral_overlay_anchored_on_the_message_square():
    ...
    assert app.game.skillcheck_session._online_skillcheck[:2] == (frm, to)
...
def test_required_wheel_placement_matches_the_pure_engine():
    ...
    assert app.game.skillcheck_session._skillcheck_target == expected
...
def test_a_result_for_a_different_move_is_ignored():
    ...
    assert app.game.skillcheck_session._online_skillcheck is not None
...
def test_spectate_collision_guard_routes_a_matching_result_to_the_spectate_branch():
    ...
    open_check = app.game.skillcheck_session._online_skillcheck
    ...
    assert app.game.skillcheck_session._online_spectate_kind is not None
...
def test_spectate_opens_a_passive_overlay_and_leaves_my_board_live():
    ...
    assert app.game.skillcheck_session._online_spectate_kind == SkillCheckKind.AIM
    ...
    assert app.game.skillcheck_session._skillcheck_swallows_input() is False, \
...
def test_resume_reopens_my_pending_check_at_the_right_elapsed():
    ...
    assert app.game.skillcheck_overlay.is_active()
    assert app.game.skillcheck_session._online_skillcheck[:2] == (sq(4, 3), sq(3, 3))
    ctrl = app.game.skillcheck_overlay._controller
...
def test_watchdog_tears_down_a_stranded_overlay_and_resyncs():
    ...
    app.game.skillcheck_session._skillcheck_gate(frm, to)
    app.coordinator._handle_skill_check_required(_required_payload(frm, to))
    app.game.skillcheck_session._online_skillcheck_opened_ms = (
...
def test_watchdog_does_not_fire_before_the_threshold():
    ...
    app.game.skillcheck_session._skillcheck_gate(frm, to)
    app.coordinator._handle_skill_check_required(_required_payload(frm, to))
    app.game.skillcheck_session._online_skillcheck_opened_ms = pg.time.get_ticks() - 2000
    app.coordinator._tick_skillcheck_watchdog()
    assert app.game.skillcheck_overlay.is_active()
...
def test_held_move_rejected_as_desync_clears_and_resyncs():
    ...
    assert app.game.skillcheck_session._pending_online_move is None, "the stranded hold is released"
    assert app.coordinator.client.state_syncs == 1
...
def test_reset_to_new_game_clears_every_online_field():
    app = _online_app()
    app.game.skillcheck_session._pending_online_move = (sq(4, 3), sq(3, 3), None)
    app.game.skillcheck_session._online_skillcheck = (
        sq(4, 3), sq(3, 3), None, SkillCheckKind.WHEEL)
    app.game.skillcheck_session._online_spectate_kind = SkillCheckKind.AIM
    app.game.skillcheck_session._online_skillcheck_opened_ms = 123
...
def test_handle_online_event_routes_spectate_and_spectate_shot():
    ...
    assert app.game.skillcheck_session._online_spectate_kind == SkillCheckKind.WHEEL
...
def test_heartbeat_is_suppressed_while_a_won_move_is_deferred_behind_the_hold():
    ...
    app.game.skillcheck_session._online_verdict_action = None
    app.coordinator._send_heartbeat_if_due()
...
def test_result_during_the_verdict_hold_flushes_the_won_movers_move():
    ...
    app.coordinator._handle_skill_check_required(_required_payload(frm, to, kind="wheel"))
    app.coordinator._handle_remote_move_applied(_move_applied(frm, to, 1, kind="wheel", won=True))
    assert app.game.skillcheck_session._online_verdict_action is not None, \
...
def test_local_shootout_still_takes_the_local_branch():
    ...
    held = app.game.skillcheck_session._skillcheck_gate(sq(4, 3), sq(3, 3))
...
def test_resumed_log_defaults_a_missing_san_to_empty():
    ...
    app = _online_app("white")
    app.game.skillcheck_session._apply_resumed_skillcheck_log(
...
def test_online_takeback_drops_the_undone_plys_outcome():
    ...
    _drive_verdict_hold(app)
    assert len(app.game.match.move_history) == 1
    assert len(app.game.skillcheck_session._skillcheck_log) == 1
...
def test_a_live_log_round_trips_through_the_resume_wire_unchanged():
    ...
    app.coordinator._handle_remote_move_applied(_move_applied(frm, to, 1, kind="wheel", won=True))
    _drive_verdict_hold(app)
    live = list(app.game.skillcheck_session._skillcheck_log)
    ...
    assert other.game.skillcheck_session._skillcheck_log == live
