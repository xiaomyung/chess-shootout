...
def test_required_wheel_placement_matches_the_pure_engine():
    ...
    assert app.skillcheck_session._skillcheck_target == expected
...
def test_resume_reopens_my_pending_check_at_the_right_elapsed():
    ...
    exclusions = app.skillcheck_session._placement_exclusions(frm, to)
    engine = placement_square("s", 3, exclusions, app.board.SIZE)
    expected = to if engine is None else sq(engine[0], engine[1])
    assert app.skillcheck_session._skillcheck_target == expected, \
...
def test_reset_to_new_game_clears_every_online_field():
    app = _online_app()
    app.skillcheck_session._pending_online_move = (sq(4, 3), sq(3, 3), None)
    app.skillcheck_session._online_skillcheck = (sq(4, 3), sq(3, 3), None, SkillCheckKind.WHEEL)
    app.skillcheck_session._online_spectate_kind = SkillCheckKind.AIM
    app.skillcheck_session._online_skillcheck_opened_ms = 123
...
def test_local_shootout_still_takes_the_local_branch():
    ...
    held = app.skillcheck_session._skillcheck_gate(sq(4, 3), sq(3, 3))
...
def test_resumed_log_defaults_a_missing_san_to_empty():
    ...
    app = _online_app("white")
    app.skillcheck_session._apply_resumed_skillcheck_log([{"ply": 1, "kind": "wheel", "won": True}])
    assert app.skillcheck_session._skillcheck_log == [SkillCheckOutcome(1, "wheel", True, "")]
