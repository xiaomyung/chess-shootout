...
flanking, and the shared _square_stack refactor guard."""
...
def test_line_board_equals_nothing_full_square(size):
    nothing = fl.focus_square(size, TOP, "nothing", SHR, SGR)
    line = fl.focus_square(size, TOP, "line", SHR, SGR)
    assert line == nothing


@pytest.mark.parametrize("size", [(1000, 800), (1600, 900)])
def test_focus_board_is_centered_square(size):
    r = fl.focus_square(size, TOP, "nothing", SHR, SGR)
    w, h = size
...
def test_focus_nothing_bigger_than_normal(size):
    focus = fl.focus_square(size, TOP, "nothing", SHR, SGR).width
    assert focus > _normal_board_width(*size)


def test_time_line_rects_in_board_margins():
    board = fl.focus_square((1000, 800), TOP, "line", SHR, SGR)
