...
lockout, hits adjudicate through the engine's hit_at (grace window included), the
third hit commits a local win while deadline/quota exhaustion commits a fail that
fades the pits and taunts after a deadpan beat. Online every registered shot
relays exactly once with a keyword (row_f, col_f) target and the client never
self-commits — resolve() carries the server verdict through the shared
RESULT_HOLD. Passive mirrors swallow no input, never touch the OS cursor, stay
muted, and replay the mover's shots via spectate_shot. The OS cursor hides on
active construction and is restored on every terminal path.
"""
...
from chessshootout.frontend.skillcheck.mole_view import (
    MoleController, MOLE_VIEW_RESULT_HOLD_MS, MOLE_VIEW_FAIL_FADE_MS,
    MOLE_VIEW_FAIL_DEADPAN_MS, MOLE_VIEW_FAIL_HOLD_MS, MOLE_VIEW_PIP_OFFSET_FRAC)
...
def test_third_hit_commits_the_win_after_the_hold():
    ctrl = _mole()
    ctrl.update(800)
    ctrl.handle_event(_click(_hole_px(0)))
    ctrl.update(2000)
    ...
    ctrl.update(3200 + MOLE_VIEW_RESULT_HOLD_MS)
...
def test_deadline_commits_fail_and_taunts_after_the_deadpan():
    ...
    ctrl.update(2000 + MOLE_VIEW_FAIL_FADE_MS + MOLE_VIEW_FAIL_DEADPAN_MS)
    ctrl._audio.play_mole_taunt.assert_called_once()
    assert ctrl.done is False
...
def test_online_resolve_holds_the_verdict_then_finishes():
    ctrl = _mole(on_shot=MagicMock())
    ctrl.update(900)
    ctrl.resolve(True)
    assert ctrl.landed is True
...
def test_draw_smoke_across_states_and_sizes(cell):
    ...
    failed.draw(surf)
    failed.update(2000 + MOLE_VIEW_FAIL_FADE_MS + MOLE_VIEW_FAIL_DEADPAN_MS + 50)
    failed.draw(surf)
...
def test_same_challenge_and_script_reach_the_same_state():
    ...
    assert a._taunt_text == b._taunt_text, "the taunt pick is seeded off the challenge content"
