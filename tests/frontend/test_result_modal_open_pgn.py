...
from chessshootout.frontend.game.result_flow import GAME_NOT_SAVED_MESSAGE
...
def test_normal_game_offers_open_pgn_and_click_opens_the_saved_file(tmp_path, monkeypatch):
...
def test_save_failure_game_hides_open_pgn(tmp_path, monkeypatch):
    monkeypatch.setenv("CHESS_DATA_DIR", str(tmp_path))
    app = make_app()
    start_single_screen(app)
...
def test_zero_ply_static_result_hides_open_pgn(tmp_path, monkeypatch):
...
def test_match_found_modal_intercepts_clicks_before_the_result_menu(tmp_path, monkeypatch):
...
def test_finalize_failure_retoasts_once_at_result(tmp_path, monkeypatch):
    ...
    assert any("Could not save PGN" in m for m in shown), "the mid-game latch toast fired"
